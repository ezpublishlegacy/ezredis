    ...
    */
    ...
    function arrayQuery( $sql, $params = array(), $server = false )
    {
        ...
        if ( $this->IsConnected )
        {
            ...
            if ($ini->hasVariable('RedisCacheSettings', 'CacheMysqlSelectQuerie') && ($ini->variable('RedisCacheSettings', 'CacheMysqlSelectQuerie') == 'enabled')) {
                eZDebug::accumulatorStart( 'redis_get_query', 'redis_total', 'Redis_Queries' );
                $redis = new Redis();
                $redis->connect('localhost', 6379);
                $redis->select(2);
                $sqlHash = sha1($sql);
                $resultRedis = $redis->get($sqlHash);
                eZDebug::accumulatorStop( 'redis_get_query');
                if($resultRedis){
                    return unserialize($resultRedis);
                }
            }
            ...
            if ($ini->hasVariable('RedisCacheSettings', 'CacheMysqlSelectQuerie') && ($ini->variable('RedisCacheSettings', 'CacheMysqlSelectQuerie') == 'enabled')) {
                eZDebug::accumulatorStart( 'redis_set_query', 'redis_total', 'Redis_Queries' );
                $redis->set($sqlHash, serialize($retArray));
                eZDebug::accumulatorStop( 'redis_set_query');
            }
