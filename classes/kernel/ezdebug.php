...
-->
        ...
        if ( isset( $this->TimeAccumulatorList['redis_get_query'] ) || isset( $this->TimeAccumulatorList['redis_set_query'] ) )
        {
            if (isset( $this->TimeAccumulatorList['redis_get_query'] )){
                if ( $as_html )
                {
                    echo "<tr class='data'><td>Redis Get Queries</td><td>" .
                       $this->TimeAccumulatorList['redis_get_query']['count'] . "</td></tr>";
                }
                else
                {
                    echo "Redis Get Queries: " .
                        $this->TimeAccumulatorList['redis_get_query']['count'] . "\n";
                }
            }
            if (isset( $this->TimeAccumulatorList['redis_set_query'] )){
                if ( $as_html )
                {
                    echo "<tr class='data'><td>Redis Set Queries</td><td>" .
                       $this->TimeAccumulatorList['redis_set_query']['count'] . "</td></tr>";
                }
                else
                {
                    echo "Redis Set Queries: " .
                        $this->TimeAccumulatorList['redis_set_query']['count'] . "\n";
                }
            }
        }
